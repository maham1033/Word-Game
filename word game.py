# -*- coding: utf-8 -*-
"""
Created on Sat Apr 30 12:57:23 2022
...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    first_component = sum([SCRABBLE_LETTER_VALUES[l.lower()] for l in word])
...
def display_hand(hand):
    ...
    for letter in hand.keys():
        print(*hand[letter]*(letter,), end=' ')  # print all on the same line
...
def is_valid_word(word, hand, word_list):
    ...
    if word not in word_list:
        if '*' in word:  # handle wildcard
            """
            Make words with all vowels substituting for the wildcard, check if
            any are valid
            """
            words_with_vowels = [word.replace('*', vowel) for vowel in VOWELS]
            if (all(word not in word_list for word in words_with_vowels)):
                return False
        else:  # word without wildcard is not in world list; is invalid
            return False
    # check against hand count
    return all(hand.get(l, 0) >= word.count(l) for l in set(word))
...
def calculate_handlen(hand):
    """
    Calculates the length of the given hand.
    :param hand: The secret word that the player guessed.
    :type hand: dict
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while hand_len > 0:
        ...
        if is_valid_word(word, hand, word_list):
            new_len = calculate_handlen(hand)
            word_score = get_word_score(word, new_len)
            score += word_score
            print('"{}" earned {} points. Total: {} points\n'
                  .format(word, word_score, score))
        else:
    ...
    output = []
    if hand_len == 0:
        output.append('Ran out of letters.')
    output.extend(['Total score for this hand:', score, '\n----------'])
    print(*output)
    return score
